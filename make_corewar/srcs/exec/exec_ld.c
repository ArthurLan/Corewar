					...
					int *add_to_pc, t_params params)
{
	...
	*add_to_pc += 2 + 1;
	...
	cell_value = (arena->cell[(pos + (address % idxmod))
			% MEM_SIZE].content << 24) |
		(arena->cell[(pos + (address % idxmod) + 1) % MEM_SIZE].content << 16) |
		(arena->cell[(pos + (address % idxmod) + 2) % MEM_SIZE].content << 8) |
		(arena->cell[(pos + (address % idxmod) + 3) % MEM_SIZE].content);
